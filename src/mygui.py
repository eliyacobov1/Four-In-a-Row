...
def new_window(parent, function):
    ...
    if function is 1:
        game.com_vs_com()
        auto_game_instance(game, parent)
...
def main_menu(parent):
    ...
    com_vs_com = tk.Button(parent, bg="lavender",
                           text="Com VS Com",
                           command=lambda function=1: new_window(parent, function),
                           font=("Helvetica", 15), fg="salmon4")
    com_vs_com.pack(side=tk.BOTTOM)

    two_player_button = tk.Button(parent, bg="lavender",
                                  text="Two Players",
                                  command=lambda function=2: new_window(parent, function),
                                  font=("Helvetica", 15),
                                  fg="salmon4")
    two_player_button.pack(side=tk.BOTTOM)

    player_vs_com_button = tk.Button(parent, bg="lavender",
                                     text="Player VS Com",
                                     command=lambda function=3: new_window(parent, function),
                                     font=("Helvetica", 15), fg="salmon4")
    player_vs_com_button.pack(side=tk.BOTTOM)

    com_vs_player_button = tk.Button(parent, bg="lavender",
                                     text="Com VS Player",
                                     command=lambda function=4: new_window(parent, function),
                                     font=("Helvetica", 15),
                                     fg="salmon4")
...
def final_menu(parent, previous_parent, winner):
    ...
    play_again = tk.Button(parent, text="PLAY AGAIN",
                           command=lambda: define_menu_command(),
                           font=("Helvetica", 15),
                           bg="lavender", fg="salmon4")
    play_again.pack(side=tk.BOTTOM)

    exit_game = tk.Button(parent, text="EXIT GAME",
                          command=lambda: define_exit_command(),
                          font=("Helvetica", 15), bg="lavender",
                          fg="salmon4")
    exit_game.pack(side=tk.BOTTOM)
...
class ConnectFour:
    ...
    def __init__(self, parent):
        ...
        self.label1 = tk.Label(parent, width=30, height=1, text="CONNECT FOUR",
                               font=("Helvetica", 30),
                               fg="salmon4",
                               bg="lavender")
        self.label1.place(x=-60, y=15)

        self.label2 = tk.Label(parent, width=30, height=1,
                               text=INSTRUCTIONS,
                               font=("Helvetica", 9),
                               fg="#a1dbcd",
                               bg="#383a39")
        ...
        self.canvas = tk.Canvas(self.frame,
                                width=square_length*WIDTH+square_length*2,
                                height=square_length*HEIGHT+square_length * 2,
                                borderwidth=5,
                                highlightthickness=5,
                                bg='lavender')
        self.canvas.grid(columnspan=7)

        # draw grid, create buttons
        ...
        for i in range(WIDTH):
            column_buttons.append(tk.Button(self.frame,
                                            text="Col %d" % int(i + 1),
                                            fg="#a1dbcd", bg="#383a39",
    ...
    def single_turn(self, col):
        ...
        :return: nothing
        """
        # if pc vs pc game, if so disable buttons
        ...
        # checks if player 1 is ai,if so, makes a move
        if self.game.get_current_player() == 1 and\
                type(self.player1) is AI and \
        ...
        # makes a move if player 2 is ai
        if self.game.get_current_player() == 2 and\
                type(self.player2) is AI and \
                self.game.get_winner() is None:
        ...
        if self.game.get_winner() is not None:
            # prints tie message if there is a tie
    ...
    def mark_win(self, coordinates):
        ...
        for c in coordinates:
            new_coordinate = self.mark_win_helper(c)
            new_coordinates.append(new_coordinate)

        self.canvas.create_text(new_coordinates[0], text="1",
                                font="Helvetica", fill="black")
        self.canvas.create_text(new_coordinates[1], fill="black",
                                text="2", font="Helvetica")
        self.canvas.create_text(new_coordinates[2], fill="black",
                                text="3", font="Helvetica")
        self.canvas.create_text(new_coordinates[3], fill="black",
                                text="4", font="Helvetica")

        self.canvas.update()

    def mark_win_helper(self, c):
        """
        creates the exact coordinates of the winning strike
        according to the given coordinate
        :param c: a coordinate of the winning strike as
        in the board of the class game
        :return: the coordinate in the board
        according to the given ones
        """
        x1 = c[1] * square_length + 60
        y1 = c[0] * square_length + 90
        x2 = x1 + square_length
        y2 = y1 + square_length
